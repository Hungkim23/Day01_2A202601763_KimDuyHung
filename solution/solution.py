...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Ghép lịch sử hội thoại cũ với câu hỏi mới
        ...
        # Cập nhật lịch sử và chỉ giữ lại 3 lượt gần nhất (6 messages)
...
def run_assistant(
    ...
    while True:
        # 1. Kiểm tra max_turns TRƯỚC khi gọi get_input()
        ...
        # 5. In từng chunk và ghép lại thành reply
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        # 6. Cập nhật history và duy trì tối đa 3 lượt (6 messages)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
